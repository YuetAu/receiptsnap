...
export async function getCompaniesForUser(userId: string): Promise<Company[]> {
    ...
    try {
        ...
        memberSnapshot.docs.forEach(doc => {
            if (!companiesMap.has(doc.id)) { // Avoid duplicates if user is owner AND in members array
...
export async function getInvitationsForUser(email: string): Promise<Invitation[]> {
    ...
    try {
        ...
        return snapshot.docs.map(doc => ({ id: doc.id, ...doc.data(), createdAt: (doc.data().createdAt as Timestamp)?.toDate() } as unknown as Invitation));
    } catch (error) {
